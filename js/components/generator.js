...
import { getGourmetFoodImage, extractEquipment, fetchHtmlThroughProxy, getPlausibleRecipeLink, generateDynamicFallback, getGeminiEndpoint } from "../utils.js?v=2.4";
import { simulateRecipeImport } from "./importer.js?v=2.4";
    ...
    .then(html => {
      ...
      if (firstLink) {
        ...
        simulateRecipeImport(
          ...
          onStepChange,
          (scrapedRecipe) => {
            const isDummy = scrapedRecipe && (
              scrapedRecipe.ingredients.some(ing => ing.name.includes("refer to original link")) ||
              scrapedRecipe.instructions.some(inst => inst.text.includes("Click the reference link above"))
            );
            if (scrapedRecipe && !isDummy) {
